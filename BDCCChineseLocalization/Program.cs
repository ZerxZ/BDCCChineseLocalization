...
public class Program
{
    ...
    )
    {
        ...
        foreach (var file in files)
        {
            ...
            try
            {
                ...
                var parser = GdScriptParser.Parse(await File.ReadAllTextAsync(file), Path.GetRelativePath(path, file));
                parser.Parse();
                if (!parser.HasTokens)
                {
                    continue;
    ...
    public void TestScript()
    {
         ...
         // reader.ParseStatement(script);
         var parser = GdScriptParser.Parse(script, "./HK_DatapadHackComputer.gd");
         parser.Parse();
